...
namespace QTsys.DAO
{
    class ProductDAO : DAOBase
    {
        ...
        public DataTable GetMaterialProductRelationByProductEx(String ID, int 数量)
        {
            try
            {
                //string sql = "SELECT 产品原料关系.原料编号,原材料.原料名称,原材料.单位,产品原料关系.原料数量*" + 数量 + " AS 需要原料数量,原材料.库存数量,'' AS 供应商,'0' AS 供应商余料,'0' AS 供应单价  FROM 产品原料关系 LEFT JOIN 原材料 ON 产品原料关系.原料编号=原材料.原料编号 WHERE 产品编号='" + ID + "';";
